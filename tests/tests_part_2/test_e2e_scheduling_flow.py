...
class TestEndToEndSchedulingFlow:
    ...
    def _wait_for_generation_to_finish(self, adapter: PlanixEngineAdapter, timeout_seconds: float = 5.0):
        """Wait until the background engine adapter process finishes schedule generation."""
        worker = adapter._worker_process
        assert worker is not None
        worker.join(timeout_seconds)
        assert adapter.is_generation_active() is False
        adapter.clear_finished_worker()
    ...
    def test_full_pipeline_loads_data_generates_output_and_indexes_schedules(self, tmp_path):
        ...
        self._wait_for_generation_to_finish(adapter)
    ...
    def test_engine_integration_rejects_generation_when_no_available_dates_exist(self, tmp_path):
        ...
        # Assert
        # The worker process fails before writing valid schedule content.
        # The important integration contract is that the adapter releases the background worker.
        assert adapter.is_generation_active() is False
        assert not output_path.exists() or "--- FULL SYSTEM OPTION" not in output_path.read_text(encoding="utf-8")
