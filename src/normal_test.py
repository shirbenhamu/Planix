...
def run_normal_test():
    ...
    # משתמשים בכותב שלנו עם הטיימר של ה-20 שניות
    writer = FileOutputWriter()

    # ניצור רק 2 קורסים לכל סמסטר
    courses = [
        # Fall Courses
    ...
    # נגדיר תקופות בחינה *קצרות מאוד* (רק 4 ימים לכל מועד)
    # עבור 2 קורסים ב-4 ימים, יש בדיוק 12 אופציות לכל מועד (4 * 3).
    exam_periods = [
    ...
    # המתמטיקה: 12 (סתיו א) * 12 (סתיו ב) * 12 (אביב א) * 12 (אביב ב) = 20,736 שילובים שנתיים בסך הכל.
    # זה מספר קלאסי שהמחשב יוציא לפועל מהר מאוד.
